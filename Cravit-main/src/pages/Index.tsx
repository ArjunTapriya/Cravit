...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      ...
      // If Firestore returns no active restaurants for the query and we have mock data,
      // fall back to local mock restaurants for development/demo purposes.
      if (restaurantList.length === 0 && mockRestaurants && mockRestaurants.length > 0) {
        // Attach the user's pincode to mock items so filtering by pincode still works
        const fallback = mockRestaurants.map(r => ({ ...r, pincode: userProfile?.pincode || "" , id: r.id }));
        setRestaurants(fallback as any);
      } else {
